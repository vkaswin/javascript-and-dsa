...
top five average. Sort result by IDj in
increasing order.

A student's
top five average is calculated by taking the sum of their top five scores and dividing it by 5 using
integer division.
...
export const highFive = (items: [number, number][]) => {
  items.sort((a, b) => b[1] - a[1]);

  let top5Scores = items.reduce((acc, [id, score]) => {
    if (acc[id] && acc[id].length === 5) return acc;
    if (id in acc) {
      acc[id].push(score);
    } else {
      acc[id] = [score];
  ...
  for (let key in top5Scores) {
    let avg = Math.floor(top5Scores[key].reduce((acc, curr) => acc + curr) / 5);
    average.push([+key, avg]);
  }

  return average.sort((a, b) => a[1] - b[1]);
};
